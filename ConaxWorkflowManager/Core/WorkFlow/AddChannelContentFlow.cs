...
{
    public class AddChannelContentFlow : BaseFlow
    {
        ...
        private void ProcessedIngest(RequestParameters requestParameters)
        {
            ...
            String processedDir = systemConfig.FileIngestProcessedDirectory;
            ...
            try
            {
                ...
                foreach (LanguageInfo lang in content.LanguageInfos)
                {
                    foreach (Image img in lang.Images)
                    {
                        ...
                        if (File.Exists(imgfile) && !imgs.Contains(Path.GetFileName(img.URI)))
